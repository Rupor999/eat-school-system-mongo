  ...
  <div class="text-center">
    <v-dialog v-model="dialog" width="600">
      <template v-slot:activator="{ on }">
        <v-btn @click="sendNewData" class="ma-4" color="primary" v-on="on"
  ...
  methods: {
    async sendNewData() {
      while (this.inputData.length) {
        await this.$store
          .dispatch("pupils/postPupil", this.inputData[0])
          .then(() => {
      ...
      if (this.inputData.length === 0) {
        ...
        this.$store.dispatch("pupils/getPupils");
      }
    }
  },
  computed: {
    ...
    error() {
      return this.$store.getters["pupils/getStatus"] === "error"
        ? "error"
        : "primary";
    }
  }
