...
export default {
  ...
  methods: {
    async deleteData() {
      while (this.inputData.length) {
        await this.$store
          .dispatch("categoriesBufet/deleteCategoriesBufet", this.inputData[0])
          .then(() => {
  ...
  computed: {
    inputData() {
      return this.$store.getters["categoriesBufet/getSelectedRows"];
    },
    amount() {
    ...
    error() {
      return this.$store.getters["categoriesBufet/getStatus"] === "error"
        ? "error"
        : "primary";
    }
  }
